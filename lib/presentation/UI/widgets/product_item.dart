...
class _ProductItemState extends State<ProductItem> {
  late bool isInWishlist;
  Future<void>? _wishlistFuture; // Track the pending async operation

  @override
  void initState() {
    super.initState();
    // Initialize isInWishlist based on whether the product is in the wishlist
    isInWishlist = false; // Set to false initially
    _checkWishlistStatus();
  }
  ...
  void _checkWishlistStatus() {
    ...
    if (wishlistId != null) {
      // Fetch wishlist and check if the product is in it
      _wishlistFuture = wishlistController
          .getWishlistByUserId(currentUserId!)
          .then((success) {
        if (success && wishlistController.userWishlist != null && mounted) {
          setState(() {
            isInWishlist = wishlistController.userWishlist!.productIds
                .contains(widget.product.id);
          });
  ...
  @override
  void dispose() {
    _wishlistFuture?.ignore(); // Cancel the pending async operation
    super.dispose();
  }
              ...
              child: widget.product.image.isEmpty
                  ...
                  : ClipOval(
                      child: Image.network(
                        ...
                        //       size: 40, color: Colors.red); // Error icon
                        // },
                      ),
                    ),
            ),
